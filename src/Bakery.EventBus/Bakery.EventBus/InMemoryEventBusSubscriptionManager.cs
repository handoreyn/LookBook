...
public class InMemoryEventBusSubscriptionEventManager:IEventBusSubscriptionManager
{
    ...
    public InMemoryEventBusSubscriptionEventManager()
    {
    ...
    public void AddSubscription<T, TH>() where T : IntegrationEvent where TH : IIntegrationEventHandler
    {
        var eventName = GetEventKey<T>();
        DoAddSubscription(typeof(TH), eventName, false);

        if (_eventTypes.Contains(typeof(T))) return;
    ...
    public Type GetEventTypeByName(string eventName) => _eventTypes.SingleOrDefault(t => t.Name.Equals(eventName));
